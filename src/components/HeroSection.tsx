    ...
    <section className="relative min-h-screen flex flex-col items-center pt-24 pb-16 overflow-hidden px-4">
      <div className="container relative z-10 text-center max-w-5xl">
        ...
        <div className="flex justify-center mb-32">
          <div className="w-px h-24 bg-gradient-to-b from-primary/80 to-transparent" />
        </div>

        {/* МАРКЕТИНГОВЫЙ БЛОК: ТЕПЕРЬ ОН БЛИЖЕ К КАРТИНКАМ */}
        <ScrollReveal delay={400}>
          ...
          {/* БОЛЬШАЯ СЕТКА КАРТИНОК (СНОВА БОЛЬШИЕ) */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {[
              ...
              <div key={i} className="group flex flex-col items-center gap-4">
                <div className={`w-full aspect-[4/5] rounded-[48px] overflow-hidden border-2 bg-card/50 shadow-2xl transition-all duration-500 hover:scale-[1.03] ${
                ...
                <span className={`text-xs font-bold uppercase tracking-widest ${
                  s.isSpecial ? "text-primary italic" : "text-muted-foreground"
                }`}>
                  {s.label}
                </span>
              </div>
