    ...
    <section className="relative min-h-screen flex flex-col items-center pt-16 pb-10 overflow-hidden px-4 bg-[#0a0a0c]">
      <div className="container relative z-10 max-w-7xl flex flex-col items-center">
        ...
        <div id="generator" className="mt-12 mb-16 w-full max-w-6xl mx-auto px-4 text-center">
         ...
         {/* --- ЭКРАН 5: ТОТ САМЫЙ FAQ (ВСТАВКА В КОНЕЦ) --- */}
          <div className="w-full max-w-4xl mx-auto text-left mb-32 px-4 mt-24 relative z-20">
            <h2 className="text-5xl md:text-6xl font-black uppercase mb-16 text-center tracking-tighter italic drop-shadow-lg">
              FAQ
            </h2>
            
            <Accordion type="single" collapsible className="w-full space-y-6">
              <AccordionItem value="faq-1" className="border-white/10 bg-white/[0.02] rounded-[40px] px-10 border transition-all hover:border-purple-500/30 backdrop-blur-md">
                <AccordionTrigger className="font-black py-8 uppercase text-xs tracking-[0.15em] hover:no-underline leading-tight text-white">
                  Где взять премиум стикеры тг бесплатно?
                </AccordionTrigger>
                <AccordionContent className="text-gray-400 pb-8 text-base leading-relaxed">
              ...
              <AccordionItem value="faq-2" className="border-white/10 bg-white/[0.02] rounded-[40px] px-10 border transition-all hover:border-purple-500/30">
                <AccordionTrigger className="font-black py-8 uppercase text-xs tracking-[0.15em] hover:no-underline leading-tight text-white">
                  Как сделать стикеры в МАКС и 3D лица?
                </AccordionTrigger>
                <AccordionContent className="text-gray-400 pb-8 text-base leading-relaxed">
                  Чтобы <span className="text-white font-bold italic underline">создать стикеры в макс</span>, загрузите селфи и выберите стиль. Наш ИИ создаст <span className="text-purple-400 font-bold uppercase italic text-xs tracking-widest">3D стикер лица</span>.
                </AccordionContent>
              </AccordionItem>

              <AccordionItem value="faq-3" className="border-white/10 bg-white/[0.02] rounded-[40px] px-10 border transition-all hover:border-purple-500/30">
                <AccordionTrigger className="font-black py-8 text-[11px] uppercase text-gray-500 italic tracking-[0.2em] hover:no-underline leading-tight">
                  Privacy: Безопасность данных и фото
                </AccordionTrigger>
                <AccordionContent className="text-gray-400 pb-8 text-base leading-relaxed">
                  Безопасность — наш приоритет. Все ваши фото <span className="text-white font-bold underline italic underline-offset-4 font-display">удаляются автоматически</span> сразу после генерации.
                </AccordionContent>
              </AccordionItem>

              <AccordionItem value="faq-4" className="border-white/10 bg-white/[0.02] rounded-[40px] px-10 border-purple-500/50 bg-purple-500/5 shadow-2xl">
                <AccordionTrigger className="font-black py-8 uppercase text-sm text-purple-400 italic hover:no-underline flex items-center gap-4">
                  🎁 Подарок за отзыв
                </AccordionTrigger>
                <AccordionContent className="text-gray-400 pb-8 text-base flex flex-col gap-4">
                  <span>Оставьте отзыв и получите бонусные токены на баланс!</span>
                  <a href="/dashboard" className="w-fit px-10 py-4 bg-purple-500/20 text-purple-400 rounded-2xl font-black uppercase text-[11px] tracking-widest border border-purple-500/30 hover:bg-purple-500/40 transition-all italic underline">
                    Личный кабинет →
                  </a>
                </AccordionContent>
              </AccordionItem>
        ...
        </ScrollReveal> {/* Тот самый ScrollReveal, который был в конце */}
      </div> {/* Тот самый div, который был в конце */}
    </section> {/* Та самая секция */}
  );
};
