...
const HeroSection = () => {
  ...
  const handleGenerate = async () => {
    if (!selectedFile) {
      alert("Сначала загрузите фото в блоке генератора!");
      return;
    }
    setIsGenerating(true);
    setTimeout(() => {
      setIsGenerating(false);
      alert("Магия запущена! Ваша очередь в нейросети — №1.");
    }, 2000);
  };
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center pt-16 pb-10 px-4 bg-[#0a0a0c]">
      <div className="container relative z-10 max-w-7xl flex flex-col items-center">
             ...
             <div className="flex flex-col items-center gap-3 shrink-0">
              ...
              {[
                ...
                <div key={i} className="flex flex-col items-center gap-2 shrink-0">
                  <div className="w-[120px] h-[120px] md:w-[155px] md:h-[155px] rounded-[28px] overflow-hidden border border-white/10 bg-card/50">
        ...
        <div className="mt-12 mb-20 w-full max-w-6xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="relative p-8 rounded-[36px] bg-white/[0.03] border border-white/10 overflow-hidden text-left">
                <div className="absolute -right-2 -top-2 text-9xl font-black text-white/[0.03]">{item.step}</div>
        ...
        {/* ЭКРАН 4: ГЕНЕРАТОР (ОБНОВЛЕННАЯ ЛОГИКА) */}
        <div id="generator" className="mt-12 mb-16 w-full max-w-6xl mx-auto px-4 text-center">
          ...
          <p className="text-gray-400 mb-10">Загрузи фото, выбери стиль и эмодзи.</p>
            ...
            <div className="lg:col-span-5 flex flex-col gap-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <div key={i} onClick={() => setSelectedStyle(s.name)} className={`relative aspect-square rounded-xl overflow-hidden border-2 transition-all cursor-pointer ${selectedStyle === s.name ? 'border-purple-500 scale-95 shadow-[0_0_15px_rgba(168,85,247,0.4)]' : 'border-white/5 opacity-50'}`}>
                    <img src={s.img} className="w-full h-full object-cover" alt={s.name} />
                  </div>
                ))}
              </div>
